...
int main(void)
{
    ...
    while (!App::shouldClose())
    {
        ...
        Renderer::DrawQuad({0,0.001,0}, {4, 4}, vec4(0.090, 0.059, 0.286, 1));
        while(x-w/2<4)
        {
            while(z-w/2<4)
            {
                float mixA = clamp((float)(8-(z+x))/8+0.25f, 0.f, 1.0f);
                Renderer::DrawQuad({x-2, 0.002,z-2}, {w, w}, mix(colorStart, colorEnd, mixA));
                z+=w+0.01f;
            }
            z = w/2;
            x+=w+0.01f;
        ...
        for(auto& pos : randomPos)
        {
            ...
            pos = mod(pos, {6,6});
            Renderer::DrawQuad({pos.x-3, y, pos.y-3}, {0.3f, 0.3f}, texture2);
            y+=0.0001;
        }
        Renderer::DrawQuad({0,0.1,0}, {2, 2}, texture);
#endif
