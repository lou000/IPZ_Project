    ...
    :Scene("testConnect4", false)
{
    ...
    if(!deserialized())
    {
        ...
        createNamedEntity("terrain", "terrain", vec3(-100,0,-100), vec3(1));
...
{
    // TODO: make terrain component, add texture to it,
    //       render to texture and then take the data and apply it to pos.y of the entities here
    auto terrain = getEntity("terrain");
    auto transform = terrain.getComponent<TransformComponent>();
    auto mesh = terrain.getComponent<MeshComponent>().model->meshes()[0];
